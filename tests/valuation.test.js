...
test("valuation peak/EV arithmetic matches header defaults (risk-adjusted)", () => {
  ...
  assert.ok(Math.abs(equity - (EV + 138.3)) < 0.01);
  // Equity $/sh = (EV + cash) / shares ≈ $46.96 at Q2 defaults
  assert.ok(Math.abs(ps - (EV + 138.3) / 217.6) < 0.01);
  assert.ok(Math.abs(ps - 46.96) < 0.1);
  assert.ok(Math.abs(evPerShare - EV / 217.6) < 0.01);
...
test("equity $/sh includes cash; EV/sh does not", () => {
  ...
  assert.ok(Math.abs(withCash.ps - noCash.ps - 138.3 / 217.6) < 0.01);
});
